...
function connect() {
  ...
  // 对接本地 Bun WebSocket 中转: ws://localhost:8080
  // 不再传 apiKey — 代理从 product-config.json 读取配置
  // 自动带 userId — 用于记忆功能，无感知
  const userId = getUserOrCreateId();
  ws = new WebSocket('ws://127.0.0.1:8080?userId=' + encodeURIComponent(userId));

  ws.onopen = () => {
    logEvent('WebSocket 已连接（本地 Bun 中转 ws://127.0.0.1:8080），等待 session.created ...');
  };
